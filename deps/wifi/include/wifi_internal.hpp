...
public:
  ...
  bool deinit() {
    ...
    ESP_LOGI(TAG, "Deinitializing WiFi stack...");
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not deinitialize WiFi subsystem: %s", esp_err_to_name(err));
      return false;
    }
  ...
  esp_netif_t *get_sta_netif() {
    if (!initialized_) {
      ESP_LOGE(TAG, "WiFi subsystem not initialized");
      return nullptr;
    }
    if (!sta_netif_) {
  ...
  bool get_mode(wifi_mode_t &mode) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not get WiFi mode: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
  ...
  bool set_storage(wifi_storage_t storage) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not set WiFi storage: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
  ...
  bool get_power_save(wifi_ps_type_t &ps_type) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not get WiFi power save type: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
  ...
  bool set_power_save(wifi_ps_type_t ps_type) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not set WiFi power save type: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
  ...
  bool get_max_tx_power_raw(int8_t &power_raw) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not get WiFi max TX power: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
  ...
  bool set_max_tx_power_raw(int8_t power_raw) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not set WiFi max TX power: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
  ...
  std::vector<wifi_ap_record_t> scan(uint16_t max_aps = 20, bool show_hidden = false) {
    ...
    if (err != ESP_OK) {
      ESP_LOGW(TAG, "Failed to restore WiFi mode after scan: %s", esp_err_to_name(err));
    }
  ...
  bool register_ap(const std::string &name, const WifiAp::Config &config, bool set_active = false) {
    ...
    if (ap_configs_.find(name) != ap_configs_.end()) {
      ESP_LOGW(TAG, "AP config '%s' already exists", name.c_str());
      return false;
    }
  ...
  bool register_sta(const std::string &name, const WifiSta::Config &config,
    ...
    if (sta_configs_.find(name) != sta_configs_.end()) {
      ESP_LOGW(TAG, "STA config '%s' already exists", name.c_str());
      return false;
    }
  ...
  bool get_ip_address(std::string &ip_address) {
    ...
    if (!active) {
      ESP_LOGE(TAG, "No active WiFi interface");
      return false;
    }
    ip_address = active->get_ip_address();
    if (ip_address.empty()) {
      ESP_LOGE(TAG, "Active WiFi interface has no IP address");
      return false;
    }
    return true;
  }
...
protected:
  ...
  bool stop_internal() {
    ...
    if (!active_sta_name_.empty() && sta_) {
      ESP_LOGI(TAG, "Stopping active STA '%s'", active_sta_name_.c_str());
      sta_->disconnect();
      active_sta_name_.clear();
      stopped = true;
    }
    ...
    if (!active_ap_name_.empty() && ap_) {
      ESP_LOGI(TAG, "Stopping active AP '%s'", active_ap_name_.c_str());
      ap_->stop();
      active_ap_name_.clear();
      stopped = true;
    }
  ...
  bool switch_to_ap_internal(const std::string &name) {
    ESP_LOGD(TAG, "Switching to AP config for name '%s'", name.c_str());

    std::lock_guard<std::recursive_mutex> lock(mutex_);
    auto it = ap_configs_.find(name);
    if (it == ap_configs_.end()) {
      ESP_LOGE(TAG, "AP config '%s' not found", name.c_str());
      return false;
    }
    ...
    if (!ap_) {
      ...
      if (!ap_->reconfigure(config)) {
        ESP_LOGE(TAG, "Failed to reconfigure WifiAp");
        return false;
      }
      // since we're reconfiguring an existing AP, we need to start it again.
      ESP_LOGI(TAG, "Starting AP '%s'", name.c_str());
      if (!ap_->start()) {
        ESP_LOGE(TAG, "Failed to start WifiAp after reconfiguration");
        return false;
      }
    }
    ESP_LOGD(TAG, "Switched to AP config '%s'", name.c_str());
  ...
  bool switch_to_sta_internal(const std::string &name) {
    ESP_LOGD(TAG, "Switching to STA config for name '%s'", name.c_str());

    std::lock_guard<std::recursive_mutex> lock(mutex_);
    auto it = sta_configs_.find(name);
    if (it == sta_configs_.end()) {
      ESP_LOGE(TAG, "STA config '%s' not found", name.c_str());
      return false;
    }
    ...
    if (!sta_) {
      ...
      if (!sta_->reconfigure(config)) {
        ESP_LOGE(TAG, "Failed to reconfigure WifiSta");
        return false;
      }
      // since we're reconfiguring an existing STA, we need to start it again.
      ESP_LOGI(TAG, "Starting STA '%s'", name.c_str());
      if (!sta_->start()) {
        ESP_LOGE(TAG, "Failed to start WifiSta after reconfiguration");
        return false;
      }
    }
    active_sta_name_ = name;
    ESP_LOGD(TAG, "Switched to STA config '%s'", name.c_str());
    return true;
  }
