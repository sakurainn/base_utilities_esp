...
public:
  ...
  ~WifiAp() override {
    ...
    if (!WifiAp::stop()) {
      ESP_LOGE(TAG, "Could not stop WiFi AP");
    }
  ...
  bool get_saved_config(wifi_config_t &wifi_config) {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not get WiFi AP config: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
  ...
  uint8_t get_num_connected_stations() const {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not get station list: %s", esp_err_to_name(err));
      return 0;
    }
    ESP_LOGD(TAG, "Number of connected stations: %d", sta_list.num);
    return sta_list.num;
  }
  ...
  std::vector<std::string> get_connected_station_ips() {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not get station list with IPs: %s", esp_err_to_name(err));
      return ips;
    }
  ...
  bool reconfigure(const Config &config) {
    ESP_LOGI(TAG, "Reconfiguring WiFi AP");
    ...
    if (config.password.size() == 0) {
      ESP_LOGE(TAG, "No password provided, setting auth mode to OPEN");
    ...
    if (err == ESP_OK) {
      if (current_mode == WIFI_MODE_STA) {
        ...
        ESP_LOGD(TAG, "STA mode already active, setting mode to APSTA");
    ...
    if (current_mode != new_mode) {
      ...
      if (err != ESP_OK) {
        ESP_LOGE(TAG, "Could not set WiFi to AP: %s", esp_err_to_name(err));
        return false;
      }
    } else {
    ...
    ESP_LOGI(TAG, "WiFi AP reconfigured successfully");
    return true;
  }
  ...
  bool start() override {
    ...
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not start WiFi AP: %s", esp_err_to_name(err));
      return false;
    }
  ...
  bool stop() override {
    ...
    if (err == ESP_OK && mode == WIFI_MODE_APSTA) {
      ...
      ESP_LOGD(TAG, "In APSTA mode, switching to STA mode");
      err = esp_wifi_set_mode(WIFI_MODE_STA);
      if (err != ESP_OK) {
        ESP_LOGE(TAG, "Could not switch to STA mode: %s", esp_err_to_name(err));
        return false;
      }
      ESP_LOGI(TAG, "WiFi AP stopped, STA still running");
      return true;
    }
    ...
    err = esp_wifi_stop();
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not stop WiFi AP: %s", esp_err_to_name(err));
      return false;
    }
  ...
  bool set_ssid_and_password(const std::string &ssid, const std::string &password) {
    ...
    esp_err_t err = esp_wifi_get_config(WIFI_IF_AP, &wifi_config);
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not get current WiFi config: %s", esp_err_to_name(err));
      return false;
    }
    ...
    err = esp_wifi_set_config(WIFI_IF_AP, &wifi_config);
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not set SSID and password: %s", esp_err_to_name(err));
      return false;
    }
    ESP_LOGI(TAG, "SSID and password updated to '%s'", ssid.c_str());
    return true;
  }
...
protected:
  ...
  void event_handler(esp_event_base_t event_base, int32_t event_id, void *event_data) {
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
      ...
      ESP_LOGI(TAG, "Station join, AID=%d", event->aid); // MAC2STR(event->mac) MACSTR
    } else if (event_id == WIFI_EVENT_AP_STADISCONNECTED) {
      wifi_event_ap_stadisconnected_t *event = (wifi_event_ap_stadisconnected_t *)event_data;
      ESP_LOGI(TAG, "Station leave, AID=%d", event->aid); // MAC2STR(event->mac) MACSTR
    }
  }

  bool register_event_handlers() {
    ESP_LOGD(TAG, "Registering event handler");
    esp_err_t err = esp_event_handler_instance_register(
        WIFI_EVENT, ESP_EVENT_ANY_ID, &WifiAp::event_handler, this, &event_handler_instance_);
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not register wifi event handler: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
  ...
  bool unregister_event_handlers() {
    ESP_LOGD(TAG, "Unregistering event handler");
    esp_err_t err = esp_event_handler_instance_unregister(WIFI_EVENT, ESP_EVENT_ANY_ID,
                                                          event_handler_instance_);
    if (err != ESP_OK) {
      ESP_LOGE(TAG, "Could not unregister wifi event handler: %s", esp_err_to_name(err));
      return false;
    }
    return true;
  }
...
// std::format printing of WifiAp::Station
namespace std {
template <> struct formatter<espp::WifiAp::Station> : std::formatter<std::string> {
...
namespace std {
// std::format printing of WifiAp::Config
template <> struct formatter<espp::WifiAp::Config> : std::formatter<std::string> {
  template <typename FormatContext>
